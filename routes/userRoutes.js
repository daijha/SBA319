...
userRouter.get('/', async (req,res)=>{// to read all of the user data in the collection 
   const results = await User.find()
     res.status(200).json(results)
});

userRouter.get('/:id', async (req,res)=>{// to read only one user by id in the collection 
